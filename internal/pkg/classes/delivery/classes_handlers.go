...
type ClassesHandler struct {
	usecases classesinterfaces.ClassesUsecases
}

func NewClassesHandler(usecases classesinterfaces.ClassesUsecases) *ClassesHandler {
	return &ClassesHandler{usecases: usecases}
}

// GetClasses handles GET /api/reference/classes
func (h *ClassesHandler) GetClasses(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	l := logger.FromContext(ctx)

	query := r.URL.Query()

	filter := models.ClassFilterParams{
	...
	result, err := h.usecases.GetClasses(ctx, filter)
	if err != nil {
...
// GetClassByEngName handles GET /api/reference/classes/{engName}
func (h *ClassesHandler) GetClassByEngName(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	l := logger.FromContext(ctx)
	...
	class, err := h.usecases.GetClassByEngName(ctx, engName)
	if err != nil {
		var code int
		var status string

		switch {
		case errors.Is(err, apperrors.ClassNotFoundErr):
	...
	responses.SendOkResponse(w, class)
}
